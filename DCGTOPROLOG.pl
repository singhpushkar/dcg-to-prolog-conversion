convertdcgtoprolog(X,Y):-readFile(X,Z),makeprologList(Z,L),open(Y,write,Str),writeprologruleTofile(Str,L).
...
writerulefact(S,[X,Y|_]):-split_string(Y,"]"," ",Z1),Z1 = [F|_],appendstring(X,"(",X1),appendstring(X1,F,X2),appendstring(X2," | X], X).",S).

readFile(X,Y):- open(X,read,Str),read_fileinLineByLine(Str,Y),close(Str).
read_fileinLineByLine(Str,[]):- at_end_of_stream(Str).
read_fileinLineByLine(Str,[X|Y]):- not(at_end_of_stream(Str)), read_line_to_codes(Str,X1),split_string(X1,"-->"," .",X2),X2 = [L|T],converttoList(L,X3),read_fileinLineByLine(Str,Y).
converttoList(X,Z):- sub_atom(X, _, 1, 0, C),C == ')',!,split_string(X,")"," ",Z1),Z1 = [Z2|_],appendstring(Z2,",",Z).
converttoList(X,Z):-appendstring(X,"(",Z).
